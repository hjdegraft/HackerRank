...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    vector<int> tempApples(apples.size());
    vector<int> tempOranges (oranges.size());
    int noApples = 0, noOranges = 0;            // if don't initialize to zero, get very large numbers

    for (int i = 0; i < apples.size(); i++) {
        tempApples[i] = a+apples[i];
        if ((tempApples[i] >= s) && (tempApples[i] <= t) )
            noApples++;
    }
    for (int j = 0; j < oranges.size(); j++) {
        tempOranges[j] = b+oranges[j];
        if ((tempOranges[j] >= s) && (tempOranges[j] <= t) ) 
            noOranges++;
    }

    cout << noApples << endl << noOranges;
}
